...
export const getEvents = async () => {
    ...
    if (!accessToken || tokenCheck.error) {
      ...
      if (!code) {
        const response = await fetch(
          "https://d0arcq5113.execute-api.eu-central-1.amazonaws.com/dev/api/get-auth-url"
        );
        const result = await response.json();
  ...
  const getToken = async (code) => {
    ...
    const response = await fetch(
      'https://d0arcq5113.execute-api.eu-central-1.amazonaws.com/dev/api/token' + '/' + encodeCode
    );
